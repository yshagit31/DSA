 //link - https://www.naukri.com/code360/problems/rotated-array_1093219?leftPanelTabValue=PROBLEM
// Problem statement
...
{
   int low=0,high=arr.size()-1,mid,min=INT_MAX;
   while(low<=high)
   {
	   mid=(low+high)/2;
  //if left half is sorted update the min by min of left half
	   if(arr[mid]>= arr[low])
	   {
		   if(min>arr[low]){
           min=arr[low];
...
//if right half is sorted updte the min by min of left half
	   else
	   {
		   if(min>arr[mid]){ 
			   min=arr[mid]; 
